...
func TestSearchEmbed_CarriesPerWorkspaceJWT_NotGlobalKey(t *testing.T) {
	...
	if _, err := s.Search(context.Background(), "wsB", "auth", nil, 10, 0); err != nil {
...
func TestSearch_FailsClosedOnMintFailure(t *testing.T) {
	...
	out, err := s.Search(context.Background(), "wsA", "auth", nil, 10, 0)
...
func TestTwoTenants_DecisiveEndToEndIsolation(t *testing.T) {
	...
	pool.ExpectQuery(`page_embeddings.*<=>`).WithArgs(pgxmock.AnyArg(), "wsA", 10, (*string)(nil), 0).
		WillReturnRows(pgxmock.NewRows([]string{"page_id", "space_id", "title", "space_name", "similarity"}).
			AddRow("pgA", "spA", "Page A", "Space A", float64(0.9)))
